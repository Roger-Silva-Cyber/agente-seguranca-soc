...
def analisar_com_ia(dados_sistema, alertas_sistema, alertas_arquivos, alertas_processos):
    prompt = f"""
...
- Processos suspeitos: {dados_sistema['processos_suspeitos'][:5]}
...
def verificar_sistema():
    ...
    try:
        ...
        alertas_exfil = detector_tokens.verificar_exfiltracao(
            alertas_tokens,
            dados['conexoes_externas']
        )
        todos_alertas = alertas_sistema + alertas_arquivos + alertas_tokens + alertas_processos + alertas_exfil
        if todos_alertas:
            print(f"⚠️  ALERTAS DETECTADOS:")
            for a in todos_alertas:
                print(f"   → {a}")
            try:
                analise = analisar_com_ia(dados, alertas_sistema, alertas_arquivos, alertas_processos)
                print(f"\n🤖 ANÁLISE IA:\n{analise}")
                ...
                eventos_do_dia.append({
                    'hora': dados['hora'],
                    'alertas': todos_alertas,
                    'analise': 'IA indisponível no momento'
                })
        else:
            print(f"✅ Sistema normal — CPU: {dados['cpu']}% | RAM: {dados['ram']}% | Conexões: {dados['total_conexoes']} | Arquivos: OK | Processos: OK")
    except Exception as e:
